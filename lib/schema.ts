...
export const TABLES: TableDef[] = [
  ...
  },
  {
    ...
    description:
      "Углублённые карточки для постов, которые валидатор отметил как 'show' но с depth='shallow'. Pipeline: Perplexity Sonar ищет связанные источники → их HTML парсится в текст → Opus синтезирует богатую карточку с фактами и ссылками на источники. UNIQUE(item_id) — один enrichment на пост.",
    ...
    columns: [
      ...
      { name: "search_query", description: "Запрос, который ушёл в Perplexity. Обычно = title исходного поста." },
      { name: "search_results_json", description: "Сырой ответ Perplexity Sonar (answer text + citations)." },
      { name: "related_sources_json", description: "Массив {url, title} — что мы реально вытащили из citations." },
      { name: "synthesis_input", description: "Полный промт, отправленный в Opus (для отладки и пересборки)." },
      { name: "synthesis_output_json", description: "Сырой JSON-ответ Opus." },
      ...
      { name: "synthesized_summary", description: "Финальный связный текст карточки (3-5 абзацев)." },
      { name: "key_facts_json", description: "JSON-массив строк — ключевые факты bullet-списком." },
      { name: "sources_used_json", description: "JSON-массив {url, title, why_relevant} — источники, которые Opus реально использовал." },
      { name: "cost_cents", description: "Примерная стоимость в центах (Perplexity + Opus). Для мониторинга burn-rate." },
