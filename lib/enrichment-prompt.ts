...
export const ENRICHMENT_RESPONSE_SCHEMA_DOC = `Отвечай СТРОГО валидным JSON следующей формы (без markdown-обёртки):
{
  ...
  "article_body": string,                          // ОСНОВНОЙ ТЕКСТ СТАТЬИ В MARKDOWN — 1500-3000 слов, 5-15 параграфов
...
export function buildEnrichmentPrompt(
  ...
  const system =
    ...
    `## ТЕМЫ ПОЛЬЗОВАТЕЛЯ\n${topicsBlock}\n\n` +
    `## ПРАВИЛА СТАТЬИ\n` +
    `- Это полноценная статья, а не «карточка». 1500-3000 слов, 5-15 параграфов.\n` +
