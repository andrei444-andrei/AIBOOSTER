...
const SYSTEM_ADDONS: Record<TaskCategory, string> = {
  ...
  research:
    `Это фактический вопрос про реальный мир. Ответ структурируй и форматируй так, чтобы читалось глазами за 5 секунд:

1. ПЕРВАЯ СТРОКА — TL;DR: одно жирное предложение с прямым ответом. Никаких «по описаниям…», «по общему мнению…», «стоит отметить…». Без подводок.
2. ДАЛЬШЕ — структура под содержимое:
   • если есть факты-параметры или сравнение → таблица GFM с понятными заголовками;
   • если несколько аспектов → заголовки ## (или ###) на каждый раздел;
   • перечисления — маркированными списками, не сплошным текстом;
   • ключевые сущности (бренды, версии, даты) — **жирным**.
3. КОНЕЦ — короткая строка «Источники: <название1>, <название2>» (без URL-простыни в теле). Если использовал поиск — перечисли что цитируешь.

Запрещено: повторять вопрос, писать «как было сказано», «возможно вы имели в виду — уточните», лить общие фразы про важность темы. Если данных нет — одна строка «не нашёл данных по X», без оправданий.

Длина не ограничена, но каждое предложение должно нести факт или вывод. Лучше 10 строк структуры, чем 30 строк прозы.`,
  code:
